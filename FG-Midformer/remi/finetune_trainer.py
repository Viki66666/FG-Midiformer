...
class FinetuneTrainer:
    def __init__(self, midi_former, train_dataloader, valid_dataloader, test_dataloader, layer,
                 lr, class_num, hs, testset_shape, cpu, cuda_devices=None, model=None, SeqClass=False, mode="mlm"):
        ...
        if model != None:  # load model
            ...
            if SeqClass:
                self.model = SequenceClassification(self.midi_former, class_num, hs, mode=mode).to(self.device)
            else:
                self.model = TokenClassification(self.midi_former, class_num, hs, mode=mode).to(self.device)

        #        for name, param in self.model.named_parameters():
        #            if 'midi_former.former' in name:
        ...
        self.scheduler = get_linear_schedule_with_warmup(
            self.optim,
            num_warmup_steps=len(train_dataloader) * 15,  # 假设warmup阶段是1个epoch
            num_training_steps=len(train_dataloader) * 50  # 假设总训练10个epoch
        )
    ...
    def iteration(self, training_data, mode, seq):
        ...
        for x, y in pbar:  # (batch, 512, 768)
            ...
            if not seq:
                ...
                attn = torch.ones((batch, x_len)).to(self.device)  # attend each of them
    ...
    def save_checkpoint(self, epoch, train_acc, valid_acc,
        ...
        state = {
            ...
            'scheduler': self.scheduler.state_dict()  # 保存调度器的状态
        }
